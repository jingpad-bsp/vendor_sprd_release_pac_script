...
if($number ne 0){
    die "\nInvalid parameters, param number must be 0.\n\n";
}

#my $pac_file    = ${ARGV[0]};
my $conf_file    = "pac.ini";
my $proj_type    = "pac_list";

print "Loading config file...\n";
my %ConfigInfo= Parse_config_file($conf_file);
my $config          = $ConfigInfo{'pac_list'}{'CONFILE'};
my $pac_prj         = $ConfigInfo{'Setting'}{'PAC_PRODUCT'};
my $pac_ver         = $ConfigInfo{'Setting'}{'ProductVer'};
my $pac_file        = $ConfigInfo{'Attr'}{'PAC_NAME'};
my $Current_ProcDir = $ConfigInfo{'Attr'}{'WORK_DIR'};

$config =~ s/\\/\//g;
if($config && (substr($config,0,1) eq '.') )
{
    $config = $Current_ProcDir.'/'.$ConfigInfo{'pac_list'}{'CONFILE'};
}
...
{
    print " partition $param[$i][0],$param[$i][1],$param[$i][2],$param[$i][3],$param[$i][4]\n";
}
print "\n";
...
do{use bytes; $szVersion = $tmp.(pack("C",0) x (44 - length($tmp)));};
...
print $fhTarget $dwHiSize;
print $fhTarget $dwLoSize;
...
if( (($offset>>32) & 0xFFFFFFFF) >0)
{
    seek $fhTarget,0,SEEK_SET;
    $tmp = MakeUnicode("BP_R2.0.1");
    do{use bytes; $szVersion2 = $tmp.(pack("C",0) x (44 - length($tmp)));};
    print $fhTarget $szVersion2;
}

seek $fhTarget,44,SEEK_SET;
$dwHiSize = GetDWORD(($offset>>32) & 0xFFFFFFFF); 
$dwLoSize = GetDWORD($offset & 0xFFFFFFFF); 
print $fhTarget $dwHiSize;
print $fhTarget $dwLoSize;
...
sub WriteFileInfoHeader
{
  my $fh = shift;
    ...
    my $dwLoDataOffset;     # the offset from the packet file header to this file data
    ...
    if($file_flag != 0 && ($check_flag != 0 || $file_flag == 2))
    {
        ...
        $dwHiFileSize = GetDWORD(($file_size>>32) & 0xFFFFFFFF);
        $dwLoFileSize = GetDWORD($file_size & 0xFFFFFFFF);
    }
    else
    {
        $dwHiFileSize = GetDWORD(0);
        $dwLoFileSize = GetDWORD(0);
    }
    ...
    if($file_flag != 0 && ($check_flag != 0 || $file_flag == 2))
    {
        $dwHiDataOffset = GetDWORD(($offset>>32) & 0xFFFFFFFF);
        $dwLoDataOffset = GetDWORD($offset & 0xFFFFFFFF);  
    }
    else
    {
        $dwHiDataOffset = GetDWORD(0);
        $dwLoDataOffset = GetDWORD(0);  
    }

    $data = $data.$dwHiFileSize;
    $data = $data.$dwHiDataOffset;
    $data = $data.$dwLoFileSize;
    $data = $data.$nFileFlag;
    $data = $data.$nCheckFlag;
    $data = $data.$dwLoDataOffset;
